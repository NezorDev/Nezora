...
public class cmdHug implements Command {
    ...
    public void action(String[] args, MessageReceivedEvent event) {
        if(args.length < 1) {
            event.getTextChannel().sendMessage("Argument missing: ```" + STATIC.PREFIX + "hug @user```").queue();
            return;
        }

        if(!args[0].startsWith("@")) {
            event.getTextChannel().sendMessage("Argument wrong: ```" + STATIC.PREFIX + "hug @user```").queue();
            return;
        }
        for(Member member: event.getTextChannel().getMembers()) {
            if("@".concat(member.getUser().getName()).equals(args[0])) {
                event.getTextChannel().sendMessage(event.getAuthor().getAsMention() + " hugged " + member.getAsMention()).queue();
                Giphy giphy = new Giphy(SECRETS.GIPHYKEY);
                try {
                    SearchRandom giphyData = giphy.searchRandom("hugging");
    ...
    public void executed(boolean sucess, MessageReceivedEvent event) {
        ...
        System.out.println("CMD: hug  used by " + event.getMessage().getMember().getUser().getName());
    }
